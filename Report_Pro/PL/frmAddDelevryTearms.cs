...
{
    public partial class frmAddDelevryTearms : Form
    {
        ...
        {
            DataTable dt_ = dal.getDataTabl_1("select * from "+dal.db1+ ".dbo.WH_PO_DELEVRY_CODE where DeLEVRY_CODE like '%" + txtCode.Text + "%' and DeLEVRY_Name like '%" + txtDesc.Text + "%' and DELEVER_NAME_E like '%" + txtDesc_L.Text + "%'");
            dt.Clear();
            int i = 0;
            DataRow row = null;
        ...
        private void fillDG()
        {
            DataTable dt_ = dal.getDataTabl_1("select * from  " + dal.db1 + ".dbo.WH_PO_DELEVRY_CODE");
            dt.Clear();
            int i = 0;
            DataRow row = null;
            foreach (DataRow r in dt_.Rows)
