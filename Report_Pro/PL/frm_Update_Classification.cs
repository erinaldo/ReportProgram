...
namespace Report_Pro.PL
{
    public partial class frm_Update_Classification : Form
    {
        ...
        {
            if (OldCat.ID.Text.Trim() != string.Empty && NewCat.ID.Text.Trim() != string.Empty)
            {
        ...
        {
            if (OldAcc.ID.Text.Trim() != string.Empty && NewAcc.ID.Text.Trim() != string.Empty)
            {
                dal.Execute_1(@"update daily_transaction set acc_no ='" + NewAcc.ID.Text + "' where acc_no='" + OldAcc.ID.Text + "' ");
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            where A.ACC_NO ='" + OldAcc.ID.Text+ "' order by A.g_date ");
            if (dtp.Rows.Count > 0)
            {
