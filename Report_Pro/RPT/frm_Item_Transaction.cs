    ...
    public partial class frm_Item_Transaction : Form
    {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
           ...
           , ROW_NUMBER()OVER(  ORDER BY  D.G_DATE,D.SER_NO ,D.BRANCH_CODE) " +
            ...
            dt2 = (dal.getDataTabl_1(@"select SUM(CASE WHEN  D.ITEM_NO like '" + UC_Items.ID.Text + "%' and cast(D.G_DATE as date) <'" + FromDate_.Value.ToString("yyyy/MM/dd") + "' and cyear= '" + FromDate_.Value.ToString("yy") + "' THEN  QTY_ADD-QTY_TAKE else 0 END) AS 'B_balance' from wh_material_transaction As D"));
            ds.Tables.Add(dt1);
            ds.Tables.Add(dt2);
