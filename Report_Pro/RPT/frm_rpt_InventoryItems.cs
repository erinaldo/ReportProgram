...
namespace Report_Pro.RPT
{
    ...
    public partial class frm_rpt_InventoryItems : Form
    {
        ...
        void createDattable()
        {
            ...
            dt_.Columns.Add("Balance_weight", typeof(decimal));
            dt_.Columns.Add("Sales_weight", typeof(decimal));
  ...
  Cursor.Current = Cursors.WaitCursor;
        ...
        "%'group by    D.item_no ,A.descr ,A.Descr_eng order by D.item_no");
            ...
            foreach (DataRow r in dt1.Rows)
            {
                ...
                row[2] = dt1.Rows[i]["Balance_Weight"].ToString().ToDecimal();
                row[3] = dt1.Rows[i]["Sales_Weight"].ToString().ToDecimal();
                row[4] = dt1.Rows[i]["Sales_Weight"].ToString().ToDecimal() * 30 / days;
                if (dt1.Rows[i]["Sales_Weight"].ToString().ToDecimal() != 0)
                {
                    row[5] = row[2].ToString().ToDecimal() / row[4].ToString().ToDecimal();
                }
                else
                {
                    row[5] = "0".ToDecimal();
